...
fn build_graph<'a>(movies: &'a [(&'a str, Vec<&'a str>)]) -> HashMap<&'a str, HashSet<&'a str>> {
...
fn find_shared_movie<'a>(
    movies: &'a [(&str, Vec<&str>)],
    actor_a: &str,
    actor_b: &str,
) -> Option<&'a str> {
    movies.iter().find_map(|(title, cast)| {
        if cast.contains(&actor_a) && cast.contains(&actor_b) {
            Some(*title)
        } else {
            None
        }
...
fn main() {
    ...
    for (start, goal) in queries {
        ...
        match bfs_shortest_path(&graph, start, goal) {
            Some(path) => {
